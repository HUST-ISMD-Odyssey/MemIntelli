...
class Qwen2Attention(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        # Separate output projection remains the same
        self.o_proj = nn.Linear(self.num_heads * self.head_dim, self.hidden_size, bias=False)
    ...
    def forward(
        ...
        # Reshape and transpose (same as original implementation)
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)

        # Rest of the method remains the same as the original implementation
        kv_seq_len = key_states.shape[-2]
...
def qwen2_5_zoo(pretrained=False, model_name="qwen2.5-0.5b"):
    config_map = {
        "qwen2.5-0.5b": {
                "attention_dropout": 0.0,
                "bos_token_id": 151643,
    ...
    #     raise ValueError(f"Model name must be one of {list(config_map.keys())}")
    config = Qwen2Config(**config_map[model_name])
